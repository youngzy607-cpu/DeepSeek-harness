...
`v0.2.0` 将 Git `plugins/` 目录作为插件同步来源：备份时自动发现有效的 `dsh-*` 自定义插件并写入清单；同步时按仓库目录复制插件、更新当前电脑的本地链接依赖并安装。界面会显示本次实际同步的插件列表。

`v0.2.1` 以 Git 仓库为唯一插件标准：`plugins/` 中每个有效的 `dsh-*` 插件都会同步到当前 Harness，不再根据本机排除名单跳过插件。

`v0.2.2` 同步 Git 后会自动补齐仓库插件的 Harness 加载条目，并将当前的自定义指令与 Skill 管理插件纳入启停面板；避免“文件已同步但未加载”。
